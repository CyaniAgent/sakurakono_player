...
class DynMentionPanel extends StatefulWidget {
  ...
  static Future<Object? /* ListOr<CoreMentionItem> */> onDynMention(
...
class _DynMentionPanelState
  ...
  Widget _buildBody(
    ThemeData theme,
    LoadingState<List<CoreMentionGroup>?> loadingState,
  ) {
    return switch (loadingState) {
      ...
      Success<List<CoreMentionGroup>?>(:final response) =>
        response != null && response.isNotEmpty
            ? SliverMainAxisGroup(
                slivers: response.map((group) {
                  if (group.items?.isNullOrEmpty != false) {
