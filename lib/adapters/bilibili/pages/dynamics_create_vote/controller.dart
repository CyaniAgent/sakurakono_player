...
class CreateVoteController extends GetxController {
  ...
  final RxList<CoreOption> options = <CoreOption>[
  ...
  Future<void> onCreate() async {
    ...
    final res = await (voteId == null
        ? Get.find<DynamicsRepository>().createVote(voteInfo)
        : Get.find<DynamicsRepository>().updateVote(voteInfo));
    if (res case Success(:final response)) {
  ...
  Future<void> onUpload(int index, String path) async {
    final res = await Get.find<MsgRepository>().uploadBfs(
      path: path,
      category: 'daily',
      biz: 'vote',
    );
