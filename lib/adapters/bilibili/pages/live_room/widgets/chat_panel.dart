...
class LiveRoomChatPanel extends StatelessWidget {
            ...
            child: TextButton(
              onPressed: () {
                ...
                final item = CoreSuperChatItem(
                  ...
                  token: '',
                  ts: now,
                  userInfo: CoreSuperChatUserInfo(
                    face: '',
                    uname: 'UNAME',
                    nameColor: '#666666',
                  ),
  ...
  InlineSpan _buildMsg(double devicePixelRatio, CoreDanmakuMsg obj) {
    final uemote = obj.uemote;
    if (uemote != null) {
      // "room_{{room_id}}_{{int}}" , "upower_[{{emote}}]" , "official_{{int}}"
      final double width, height;
      if (uemote.emoticonUnique.startsWith('official_')) {
