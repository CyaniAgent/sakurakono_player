...
class _PgcIndexPageState extends State<PgcIndexPage>
  ...
  Widget _buildBody(
    ThemeData theme,
    LoadingState<CorePgcIndexConditionData> loadingState,
  ) {
  ...
  Widget _buildSortWidget(
    ThemeData theme,
    int index,
    CorePgcIndexConditionData data,
    Object item,
    Map<String, dynamic> indexParams,
  ) {
    if (item is CorePgcConditionOrder) {
    ...
    if (item is CorePgcConditionValue) {
  ...
  Widget _buildList(LoadingState<List<CorePgcIndexItem>?> loadingState) {
    return switch (loadingState) {
