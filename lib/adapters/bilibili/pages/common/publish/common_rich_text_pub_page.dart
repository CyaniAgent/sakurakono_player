...
import 'package:skf/adapters/bilibili/models_new/emote/emote.dart' as e;
import 'package:skf/adapters/bilibili/models_new/live/live_emote/emoticon.dart';
import 'package:skf/adapters/bilibili/pages/common/publish/common_publish_page.dart';
import 'package:skf/adapters/bilibili/pages/dynamics_mention/view.dart';
        ...
        GestureDetector(
          onTap: () => PageUtils.imageView(
            imgList: imageList
                .map(
                  (img) => switch (img) {
                    FilePicModel e => CoreSourceModel(
                      url: e.path,
                      sourceType: .fileImage,
                    ),
                    OpusPicModel e => CoreSourceModel(
                      url: e.url!,
      ...
      try {
        pictures = await Future.wait<Map<String, dynamic>>(
          imageList.map((img) async {
            switch (img) {
              case FilePicModel e:
                final result = await (_ref?.read(msgRepositoryProvider) ?? appRead(msgRepositoryProvider)).uploadBfs(
                  path: e.path,
                  category: 'daily',
                  biz: 'new_dyn',
