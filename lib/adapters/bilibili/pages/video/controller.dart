...
import 'package:skf/adapters/bilibili/models_new/video/video_play_info/subtitle.dart';
...
import 'package:skf/adapters/bilibili/pages/video/medialist/view.dart';
import 'package:skf/adapters/bilibili/pages/video/note/view.dart';
import 'package:skf/adapters/bilibili/pages/video/post_panel/view.dart';
import 'package:skf/adapters/bilibili/pages/video/send_danmaku/view.dart';
...
/// Converts an adapter [VideoType] to a core [CoreVideoType].
CoreVideoType toCoreVideoType(VideoType t) {
  switch (t) {
    case VideoType.ugc:
...
/// Converts a core [CorePlayUrlModel] to an adapter [PlayUrlModel]
/// via JSON round-trip to preserve all fields including dash/durl/volume.
PlayUrlModel _corePlayUrlToAdapter(CorePlayUrlModel core) {
  return PlayUrlModel.fromJson(<String, dynamic>{
    'from': core.from,
    'result': core.result,
...
class VideoDetailController extends GetxController
  ...
  void _setVideoHeight() {
    try {
      ...
      final isVertical = (height ?? 0) > (width ?? 0);
  ...
  Future<void> getMediaList({
    ...
    if (res case Success(:final response)) {
      if (response.mediaList.isNotEmpty) {
        final converted = response.mediaList
            .map((e) => MediaListItemModel(
                  aid: e.aid,
                  bvid: e.bvid,
                  cover: e.cover,
                  title: e.title,
  ...
  Future<void> queryVideoUrl({
    ...
    if (result case Success(:final response)) {
      data = _corePlayUrlToAdapter(response);
  ...
  Future<void> onCast() async {
    ...
    if (res case Success(:final response)) {
      ...
      final List<String> playUrls = [
        first['url'] as String,
        if (first['backup_url'] is List) ...(first['backup_url'] as List).cast<String>(),
      ];
      final url = VideoUtils.getCdnUrl(playUrls);
