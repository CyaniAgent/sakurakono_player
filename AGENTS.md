...
- **GetX (legacy, being removed)**: `GetMaterialApp`, `GetPage`, `Get.find` still present in adapter pages. Migration to Riverpod `ref.watch`/`ref.read` + `ListenableBuilder` is ongoing. As of 2026-08: **Obx eliminated** (0), **`.obs` eliminated** (0), **`Get.find` reduced to ~203 calls in 81 files** (down from ~437), `import 'package:get/get.dart'` in ~250 files. Remaining `Get.find` blocks: tag-based lookups needing family providers, Host interface lookups, controllers without `_ref`, State class constraints. New controllers use `ChangeNotifier` + `notifyListeners`.
