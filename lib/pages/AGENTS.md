...
- 7 abstract interfaces define adapter-dependent behavior; pages resolve them at runtime via `Get.find<T>()` (GetX) or Riverpod providers (hybrid migration in progress):
...
- common/ 14: shared page infra — common_controller.dart (CommonController<R,T> extends GetxController with ScrollOrRefreshMixin), common_page.dart (CommonPageState), common_list_controller, common_data_controller, fab_mixin, cover_ratio, bar_hide_type, msg_unread_type, dynamic_badge_mode + multi_select/, search/, slide/ subdirs.
...
- Controllers: GetX legacy (`Get.find`/`Get.put`/`Obx`) transitioning to Riverpod StateNotifier (hybrid `_ref?.read(provider) ?? Get.find<T>()`).
- Data access via core repository interfaces (GetX DI or Riverpod providers) — never direct HTTP.
