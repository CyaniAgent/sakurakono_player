...
import 'package:skf/adapters/bilibili/pages/pgc/widgets/pgc_card_v_timeline.dart';
import 'package:skf/adapters/bilibili/pages/pgc_index/controller.dart';
import 'package:skf/adapters/bilibili/pages/pgc_index/view.dart';
...
class _PgcPageState extends State<PgcPage> with AutomaticKeepAliveClientMixin {
  ...
  Widget _buildTimeline(
    ThemeData theme,
    LoadingState<List<CoreTimelineResult>?> loadingState,
  ) => switch (loadingState) {
    Loading() => m3eLoading,
  ...
  Widget _buildRcmdBody(LoadingState<List<CorePgcIndexItem>?> loadingState) {
  ...
  Widget _buildFollowBody(LoadingState<List<CoreFavPgcItemModel>?> loadingState) {
    return switch (loadingState) {
      Loading() => m3eLoading,
      Success(:final response) =>
        response != null && response.isNotEmpty
