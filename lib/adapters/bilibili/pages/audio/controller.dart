...
class AudioController extends GetxController
  ...
  late int itemType;
  Int64? extraId;
  late final CoreAudioPlaylistSource from;
  @override
  ...
  Future<void> _queryPlayList({
    ...
    final res = await AudioGrpc.audioPlayList(
      from: isInit ? PlaylistSource.valueOf(from.value) : null,
      id: id,
      oid: isInit ? oid : null,
    ...
    if (res case Success(:final response)) {
      if (isInit) {
        ...
        if (response.list.isNotEmpty) {
          index = (index ?? 0) + response.list.length;
  ...
  Future<void> _onOpenMedia(
    String url, {
    String ua = BiliConstants.userAgentApp,
    String? referer,
  }) async {
  ...
  void onChangeOrder(CoreAudioListOrder value) {
    if (order != value) {
      order = value;
      _queryPlayList(isInit: true);
    }
