...
class WhisperDetailController extends CommonListControllerRiverpod<RspSessionMsg, Msg> {
  ...
  @override
  Future<LoadingState<RspSessionMsg>> customGetData() async {
    final raw = await ImGrpc.syncFetchSessionMsgs(
      talkerId: talkerId,
      beginSeqno: msgSeqno != null ? Int64.ZERO : null,
      endSeqno: msgSeqno,
    );
    final LoadingState<RspSessionMsg> state = switch (raw) {
      Loading() => LoadingState.loading(),
      Success(:final response) => Success<RspSessionMsg>(response),
