...
import 'package:skf/common/widgets/context_menu/reply_menu_helper.dart';
...
import 'package:skf/adapters/bilibili/pages/video_parts/reply/widgets/zan_grpc.dart';
...
class ReplyItemGrpc extends StatelessWidget {
  ...
  Widget _buildHeader(BuildContext context, ColorScheme colorScheme) {
    ...
    Widget header = GestureDetector(
      onTap: () {
        feedBack();
        AppNavigator.toNamed('/member?mid=${replyItem.mid}');
      },
                ...
                if (replyControl.isUpTop) ...[
                  const WidgetSpan(
                    ...
                    child: PBadge(
                      ...
                      size: CorePBadgeSize.small,
                      isStack: false,
                      type: CorePBadgeType.line_primary,
                      fontSize: 9,
                      textScaleFactor: 1,
                    ),
                  ),
      ...
      child: TextButton(
        ...
        onPressed: () async {
          if (replyControl.showTranslation) {
            ...
            isProcessing = true;
            final res = await appRead(replyRepositoryProvider).translateReply(
        ...
        if (content.emotes.containsKey(matchStr)) {
          ...
          spanChildren.add(
            TextSpan(
              ...
              recognizer: NoDeadlineTapGestureRecognizer()
                ..onTap = () =>
                    AppNavigator.toNamed('/member?mid=${content.atNameToMid[name]}'),
            ),
          );
          ...
          if (ownerMid == upMid || ownerMid == item.member.mid)
            ListTile(
              onTap: () async {
                ...
                final res = await appRead(replyRepositoryProvider).replyDel(
              ...
              onTap: () {
                ...
                autoWrapReportDialog(
                  ...
                  (reasonType, reasonDesc, banUid) async {
                    final res = await appRead(replyRepositoryProvider).report(
          ...
          ListTile(
            onTap: () {
              ...
              showReplyCopyDialog(
                ...
                message,
                Map.fromEntries(
                  item.content.emotes.entries.map((e) => MapEntry(
                    e.key,
                    EmoteData(url: e.value.url, size: 1.0),
                  )),
                ),
              );
            },
